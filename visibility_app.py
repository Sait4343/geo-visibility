...
def show_keywords_page():
    ...
    ВЕРСІЯ: FIX UNBOUND LOCAL ERROR.
    1. Виправлено помилку 'new_auto referenced before assignment'.
    2. Збережено весь функціонал (масові дії, очищення кешу).
    """
    import pandas as pd
    import streamlit as st
    from datetime import datetime
    ...
    with st.expander("✏️ Редагування запитів", expanded=False): 
        ...
        with tab_auto:
            ...
            if not allow_cron_global:
                ...
                st.markdown("""
                **ℹ️ Як це працює:**
                1. **✅ Застосувати для всіх:** Активує автозапуск (`ON`) і встановлює обрану частоту.
    ...
    for idx, k in enumerate(keywords, start=1):
        with st.container(border=True):
            ...
            with c4:
                ...
                # 🔥 FIX: Ініціалізуємо змінну перед використанням
                new_auto = is_auto 
                
                with cron_c1:
                    if allow_cron_global:
                        # Унікальний ключ для тогла
