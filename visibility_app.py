...
def show_keyword_details(kw_id):
    ...
    ВЕРСІЯ: FIXED CYRILLIC MATCHING.
    """
    import pandas as pd
    import plotly.express as px
    import streamlit as st
    ...
    # 🔥 ФУНКЦІЯ НОРМАЛІЗАЦІЇ (Для кирилиці)
    def normalize_brand_name(name):
        if not name: return ""
        s = str(name).lower()
        # Залишає тільки літери і цифри
    ...
    # ⚙️ БЛОК НАЛАШТУВАНЬ (Залишено як є)
    ...
    try:
        ...
        if not df_mentions.empty:
            def check_is_real_target(row):
                ...
                row_clean = normalize_brand_name(row.get('brand_name', ''))
    ...
    for tab, ui_model_name in zip(tabs, ALL_MODELS_UI):
        with tab:
            ...
            with st.container(border=True):
                ...
                sent_color = "#333"
                if "Позитив" in str(loc_sent): sent_color = "#00C896"
                elif "Негатив" in str(loc_sent): sent_color = "#FF4B4B"
                ...
                if raw_text:
                    ...
                    if target_brand_name:
                        hl_span = f"<span style='background-color:#dcfce7; color:#166534; font-weight:bold; padding:0 4px; border-radius:4px;'>{target_brand_name}</span>"
                        try: final_html = re.sub(re.escape(target_brand_name), hl_span, final_html, flags=re.IGNORECASE)
                        except: pass
            ...
            # --- ДЖЕРЕЛА ---
            st.markdown(f"#### 🔗 Цитовані джерела", unsafe_allow_html=True)
            try:
