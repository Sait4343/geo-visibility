...
def generate_html_report_content(project_name, df_scans, df_mentions, df_sources):
    """
    Генерує HTML-звіт з детальними таблицями брендів та джерел для кожного запиту.
    """
    import pandas as pd
    from datetime import datetime
    import json
    ...
    * { box-sizing: border-box; }
    ...
    .kpi-big-num { font-size: 24px; font-weight: 800; color: #2c3e50; margin-bottom: 10px; }
    ...
    /* ITEM METRICS (MINI DASHBOARD INSIDE) */
    .mini-kpi-row { display: flex; gap: 10px; padding: 20px; background: #fff; border-bottom: 1px solid #eee; flex-wrap: wrap; }
    .mini-kpi { flex: 1; border: 1px solid #ddd; border-radius: 8px; padding: 10px; text-align: center; min-width: 120px; }
    .mk-label { font-size: 10px; text-transform: uppercase; color: #888; display: block; margin-bottom: 5px; }
    .mk-val { font-size: 16px; font-weight: 800; color: #2c3e50; }

    /* TABLES INSIDE ACCORDION */
    .details-section { padding: 20px; display: flex; flex-wrap: wrap; gap: 20px; }
    .detail-col { flex: 1 1 300px; }
    .detail-title { font-weight: bold; font-size: 14px; margin-bottom: 10px; color: #00d18f; text-transform: uppercase; }
    
    table.inner-table { width: 100%; border-collapse: collapse; font-size: 12px; }
    table.inner-table th { background: #f1f3f5; padding: 8px; text-align: left; border-bottom: 2px solid #ddd; color: #555; }
    ...
    .item-response { background-color: #f8f9fa; color: #333; padding: 20px; font-size: 14px; line-height: 1.6; border-top: 1px solid #eee; }
    .response-label { font-weight: bold; color: #5e35b1; margin-bottom: 10px; display: block; }
...
<div class="content-card">
    <div class="virshi-logo-container">
        <img src="https://raw.githubusercontent.com/virshi-ai/image/39ba460ec649893b9495427aa102420beb1fa48d/virshi-op_logo-main.png" class="logo-img-real" alt="VIRSHI Logo">
    </div>
...
'''
    ...
    for i, prov in enumerate(providers):
        ...
        # 4.1. Фільтруємо основні дані по провайдеру
        df_p = df_scans[df_scans['provider'] == prov].copy()
        total_scans = len(df_p)
        if total_scans == 0: continue

        # 4.2. Глобальні метрики для вкладки
        scan_ids_in_prov = df_p['id'].tolist()
        
        # Фільтруємо деталі тільки для цього провайдера
        mentions_prov = df_mentions[df_mentions['scan_result_id'].isin(scan_ids_in_prov)]
        sources_prov = df_sources[df_sources['scan_result_id'].isin(scan_ids_in_prov)]
        
        # Рахуємо KPI
        pres_count = df_p[df_p['my_mentions_count'] > 0].shape[0]
        pres_pct = (pres_count / total_scans * 100)
        
        tot_links = sources_prov.shape[0]
        off_links = sources_prov[sources_prov['is_official'] == True].shape[0]
        off_pct = (off_links / tot_links * 100) if tot_links > 0 else 0
        
        tot_brands_mentions = mentions_prov['mention_count'].sum()
        my_mentions_sum = mentions_prov[mentions_prov['is_my_brand'] == True]['mention_count'].sum()
        sov_pct = (my_mentions_sum / tot_brands_mentions * 100) if tot_brands_mentions > 0 else 0
        ...
        for idx, row in df_p.iterrows():
            ...
            my_m_rows = local_mentions[local_mentions['is_my_brand'] == True]
            loc_my_count = my_m_rows['mention_count'].sum()
            loc_total_count = local_mentions['mention_count'].sum()
            
            loc_sov = (loc_my_count / loc_total_count * 100) if loc_total_count > 0 else 0
            
            # Позиція
            loc_rank_val = "-"
            if not my_m_rows.empty:
                val = my_m_rows[my_m_rows['rank_position'] > 0]['rank_position'].min()
                if pd.notnull(val): loc_rank_val = f"#{val}"
            
            # Тональність
            loc_sent_val = "Нейтральний"
            if not my_m_rows.empty:
                # Беремо найчастішу або першу
                mode = my_m_rows['sentiment_score'].mode()
                if not mode.empty: loc_sent_val = mode[0]

            # --- ГЕНЕРАЦІЯ ТАБЛИЦІ БРЕНДІВ HTML ---
            brands_table_html = ""
            if not local_mentions.empty:
                # Сортуємо: мої перші, потім по кількості
                local_mentions_sorted = local_mentions.sort_values(by=['is_my_brand', 'mention_count'], ascending=[False, False])
                brands_rows = ""
                for _, bm in local_mentions_sorted.iterrows():
                    style = "style='background-color:#e6fffa; font-weight:bold;'" if bm['is_my_brand'] else ""
                    brands_rows += f"<tr {style}><td>{bm['brand_name']}</td><td align='center'>{int(bm['mention_count'])}</td><td align='center'>{bm.get('sentiment_score','-')}</td><td align='center'>{int(bm.get('rank_position',0))}</td></tr>"
                
                brands_table_html = f"""
                <div class="detail-col">
                    <div class="detail-title">Знайдені бренди</div>
                    <table class="inner-table">
                        <thead><tr><th>Бренд</th><th>Згадок</th><th>Тональність</th><th>Поз.</th></tr></thead>
                        <tbody>{brands_rows}</tbody>
                    </table>
                </div>
                """
            else:
                brands_table_html = "<div class='detail-col'><p style='color:#999; font-size:12px;'>Брендів не знайдено.</p></div>"

            # --- ГЕНЕРАЦІЯ ТАБЛИЦІ ДЖЕРЕЛ HTML ---
            sources_table_html = ""
            if not local_sources.empty:
                src_rows = ""
                # Показуємо топ-10 джерел
                for _, src in local_sources.head(10).iterrows():
                    is_off_icon = "✅" if src.get('is_official') else "🔗"
                    url_short = str(src['url'])
                    if len(url_short) > 40: url_short = url_short[:37] + "..."
                    src_rows += f"<tr><td><a href='{src['url']}' target='_blank' style='color:#00d18f; text-decoration:none;'>{url_short}</a></td><td align='center'>{is_off_icon}</td></tr>"
                
                sources_table_html = f"""
                <div class="detail-col">
                    <div class="detail-title">Цитовані джерела</div>
                    <table class="inner-table">
                        <thead><tr><th>URL</th><th>Тип</th></tr></thead>
                        <tbody>{src_rows}</tbody>
                    </table>
                </div>
                """
            else:
                sources_table_html = "<div class='detail-col'><p style='color:#999; font-size:12px;'>Джерел не знайдено.</p></div>"

            # Текст відповіді
            raw_text = row.get('raw_response', '')
            if pd.isna(raw_text) or not raw_text: raw_text = "Текст відповіді не збережено."
            ...
            <div class="item-box">
                ...
                <div class="accordion-content" style="display:none;">
                    ...
                    <div class="mini-kpi-row">
                        <div class="mini-kpi"><span class="mk-label">Частка (SOV)</span><span class="mk-val">{loc_sov:.1f}%</span></div>
                        <div class="mini-kpi"><span class="mk-label">Згадок нас</span><span class="mk-val">{int(loc_my_count)}</span></div>
                        <div class="mini-kpi"><span class="mk-label">Тональність</span><span class="mk-val">{loc_sent_val}</span></div>
                        <div class="mini-kpi"><span class="mk-label">Позиція</span><span class="mk-val">{loc_rank_val}</span></div>
                    </div>
                    ...
                    <div class="item-response">
                        <span class="response-label">Відповідь {prov}:</span>
    ...
    final_html = html_template \
        .replace("__CSS_PLACEHOLDER__", css_styles) \
...
def show_reports_page():
    """
    Сторінка Звітів (Фінальна логіка).
    1. ЗБІР ДАНИХ: Бере останні сканування для КОЖНОГО запиту по КОЖНІЙ моделі.
    2. ПОТІК: Генерація -> Pending -> Публікація Admin -> Published.
    """
    import streamlit as st
    import pandas as pd
    from datetime import datetime
    ...
    # Тільки адміни бачать вкладку модерації
    ...
    # ==============================================================================
    # TAB 1: ЗАМОВЛЕННЯ ЗВІТУ (ГЕНЕРАЦІЯ ПО ВСІХ ЗАПИТАХ)
    # ==============================================================================
    with tabs[0]:
        st.markdown("### Створення нового звіту")
        st.info("Звіт буде згенеровано на основі **найактуальніших** сканувань для **всіх** запитів проекту по **всіх** доступних моделях.")
        
        default_name = f"Звіт {proj.get('brand_name')} - {datetime.now().strftime('%d.%m.%Y')}"
        new_rep_name = st.text_input("Назва звіту", value=default_name)
        
        if st.button("🚀 Згенерувати звіт", type="primary"):
            with st.spinner("Збір найсвіжіших даних по всіх запитах..."):
                try:
                    # 1. Завантажуємо Keywords (щоб мати мапу ID -> Текст)
                    kw_resp = supabase.table("keywords").select("id, keyword_text").eq("project_id", proj["id"]).execute()
                    kw_map = {k['id']: k['keyword_text'] for k in kw_resp.data} if kw_resp.data else {}
                    
                    if not kw_map:
                        st.error("У проекті немає запитів. Спочатку додайте запити.")
                        st.stop()

                    # 2. Завантажуємо ВЕЛИКИЙ пул останніх сканувань (наприклад, 2000)
                    # Щоб гарантовано охопити всі запити * всі моделі
                    scans_resp = supabase.table("scan_results")\
                    ...
                    if not raw_scans:
                        st.error("Історія сканувань порожня. Запустіть сканування у розділі 'Перелік запитів'.")
                        st.stop()
                        
                    # 3. ЛОГІКА "ЗРІЗУ": Залишаємо тільки 1 найновіший запис для пари (keyword_id + provider)
                    df_raw = pd.DataFrame(raw_scans)
                    
                    # Сортуємо: нові зверху
                    df_raw = df_raw.sort_values('created_at', ascending=False)
                    
                    # Видаляємо дублікати, залишаючи перший (найновіший) для кожного комбінації
                    df_latest = df_raw.drop_duplicates(subset=['keyword_id', 'provider'], keep='first').copy()
                    
                    scan_ids = df_latest['id'].tolist()
                    st.write(f"🔍 Знайдено {len(scan_ids)} актуальних зрізів даних (Запит + LLM).")
                    
                    # 4. Підтягуємо деталі (Mentions & Sources) тільки для цих ID
                    # Використовуємо chunks, якщо ID дуже багато, але для <1000 ок
                    ...
                    # 5. Приєднуємо дані (Покроково, щоб уникнути помилки suffixes)
                    df_latest['keyword'] = df_latest['keyword_id'].map(kw_map).fillna("Видалений запит")
                    try:
                        df_latest['created_at_dt'] = pd.to_datetime(df_latest['created_at'])
                    except:
                        pass

                    # --- MERGE MENTIONS ---
                    if not mentions_df.empty:
                        # Всього брендів у відповіді
                        bc = mentions_df.groupby('scan_result_id').size().reset_index(name='total_brands')
                        df_latest = df_latest.merge(bc, left_on='id', right_on='scan_result_id', how='left')
                        # Чистимо дублікат колонки
                        if 'scan_result_id' in df_latest.columns: 
                            df_latest = df_latest.drop(columns=['scan_result_id'])
                        
                        # Наші згадки
                        my = mentions_df[mentions_df['is_my_brand']==True].groupby('scan_result_id')['mention_count'].sum().reset_index(name='my_mentions_count')
                        df_latest = df_latest.merge(my, left_on='id', right_on='scan_result_id', how='left')
                        if 'scan_result_id' in df_latest.columns: 
                            df_latest = df_latest.drop(columns=['scan_result_id'])
                        
                        # Середня позиція (Rank)
                        ranks = mentions_df[mentions_df['is_my_brand']==True][['scan_result_id', 'rank_position']]
                        # Якщо кілька згадок, беремо кращу (мін) або середню
                        ranks_agg = ranks.groupby('scan_result_id')['rank_position'].mean().reset_index(name='rank_position')
                        df_latest = df_latest.merge(ranks_agg, left_on='id', right_on='scan_result_id', how='left')
                        if 'scan_result_id' in df_latest.columns: 
                            df_latest = df_latest.drop(columns=['scan_result_id'])

                    else:
                        df_latest['total_brands'] = 0
                        df_latest['my_mentions_count'] = 0
                        df_latest['rank_position'] = 0
                        
                    # --- MERGE SOURCES ---
                    if not sources_df.empty:
                        lc = sources_df.groupby('scan_result_id').size().reset_index(name='total_links')
                        df_latest = df_latest.merge(lc, left_on='id', right_on='scan_result_id', how='left')
                        if 'scan_result_id' in df_latest.columns: 
                            df_latest = df_latest.drop(columns=['scan_result_id'])
                        
                        oc = sources_df[sources_df['is_official']==True].groupby('scan_result_id').size().reset_index(name='official_links')
                        df_latest = df_latest.merge(oc, left_on='id', right_on='scan_result_id', how='left')
                        if 'scan_result_id' in df_latest.columns: 
                            df_latest = df_latest.drop(columns=['scan_result_id'])
                    else:
                        df_latest['total_links'] = 0
                        df_latest['official_links'] = 0
                        
                    df_latest = df_latest.fillna(0)
                    
                    # 6. ГЕНЕРАЦІЯ HTML
                    # Тут ми викликаємо функцію, яка має бути оголошена ВИЩЕ
                    if 'generate_html_report_content' in globals():
                        html_code = generate_html_report_content(proj.get('brand_name'), df_latest)
                        
                        # 7. ЗБЕРЕЖЕННЯ В БАЗУ (Pending)
                        supabase.table("reports").insert({
                            "project_id": proj["id"],
                            "report_name": new_rep_name,
                            "html_content": html_code,
                            "status": "pending" # <-- ВАЖЛИВО: Спочатку статус 'pending'
                        }).execute()
                        
                        st.success("✅ Звіт успішно сформовано! Він надісланий на модерацію (Вкладка 'Адмінка').")
                    else:
                        st.error("Функція генерації HTML не знайдена в коді.")
                    
                except Exception as e:
                    st.error(f"Помилка при генерації звіту: {e}")

    # ==============================================================================
    # TAB 2: ГОТОВІ ЗВІТИ (Тільки Published)
    # ==============================================================================
    with tabs[1]:
        ...
        try:
            ...
            if not reports:
                st.info("У цьому проекті ще немає опублікованих звітів.")
            else:
                for r in reports:
                    date_display = r['created_at'][:10]
                    with st.expander(f"📄 {r['report_name']} ({date_display})"):
                        c_down, c_view = st.columns([1, 2])
                        with c_down:
                            st.download_button(
                                label="📥 Завантажити HTML",
                                data=r['html_content'],
                                file_name=f"report_{date_display}.html",
                                mime="text/html",
                                key=f"dl_{r['id']}"
                            )
                        with c_view:
                            if st.checkbox("Переглянути", key=f"sh_{r['id']}"):
                                st.components.v1.html(r['html_content'], height=800, scrolling=True)
        except Exception as e:
            if "permission denied" in str(e):
                st.error("⚠️ Помилка доступу до таблиці 'reports'. Виконайте SQL Migration.")
            else:
                st.error(f"Помилка завантаження: {e}")

    # ==============================================================================
    # TAB 3: АДМІНКА (Тільки для Admin -> Редагування Pending)
    # ==============================================================================
    if is_admin:
        with tabs[2]:
            st.markdown("### ⚙️ Модерація звітів (Pending)")
            st.caption("Тут відображаються звіти, які були згенеровані, але ще не підтверджені.")
            ...
            try:
                ...
                if not pending:
                    st.info("Черга на модерацію пуста.")
                else:
                    for pr in pending:
                        st.divider()
                        st.subheader(f"📝 {pr['report_name']}")
                        
                        # РЕДАКТОР КОДУ (Можливість правити HTML перед публікацією)
                        ...
                        with st.expander("👁️ Попередній перегляд (Live Preview)"):
                            st.components.v1.html(edited_html, height=500, scrolling=True)
                        
                        c_pub, c_del = st.columns([1, 4])
                        with c_pub:
                            if st.button("✅ Опублікувати", key=f"pub_{pr['id']}", type="primary"):
                                # Змінюємо статус на 'published'
                                ...
                                st.success("Звіт опубліковано! Він тепер доступний у вкладці 'Готові звіти'.")
                                st.rerun()
                                
                        with c_del:
                            if st.button("❌ Видалити чернетку", key=f"del_{pr['id']}"):
                                supabase.table("reports").delete().eq("id", pr['id']).execute()
                                st.warning("Чернетку видалено.")
                                st.rerun()
