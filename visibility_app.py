...
def show_keywords_page():
    ...
    ВЕРСІЯ: FULL BULK SYNC FIX.
    1. Виправлено візуальне оновлення чекбоксів при масових діях (Clear Session State).
    2. Виправлено UnboundLocalError (new_auto).
    3. Додано кнопку "Вимкнути все".
    """
    import pandas as pd
    import streamlit as st
    from datetime import datetime
        ...
        with tab_auto:
            ...
            if not allow_cron_global:
                ...
                with c_btn:
                    ...
                    # КНОПКА: УВІМКНУТИ ВСЕ + ЗАСТОСУВАТИ ЧАСТОТУ
                    if st.button("✅ Застосувати частоту та Увімкнути", type="primary", use_container_width=True):
                        try:
                            ...
                            # 2. 🔥 ОЧИЩЕННЯ СТАНУ ВІДЖЕТІВ (FIX)
                            # Це змушує чекбокси внизу сторінки оновитися, взявши значення з БД
                            for key in list(st.session_state.keys()):
                ...
                if st.button("⛔ Вимкнути автосканування для всіх", use_container_width=True):
                     try:
                        ...
                        st.warning("Автосканування вимкнено для всіх запитів проекту.")
                        time.sleep(1.5)
                        st.rerun()
                     except Exception as e:
                        st.error(f"Помилка: {e}")
                ...
                st.markdown("""
                **ℹ️ Як це працює:**
                1. **✅ Застосувати:** Активує автозапуск (`ON`) і встановлює обрану частоту для **всіх** запитів. Чекбокси внизу сторінки автоматично перейдуть у стан "Увімкнено".
                2. **⛔ Вимкнути все:** Деактивує автозапуск (`OFF`) для всіх запитів. Чекбокси внизу сторінки вимкнуться.
                3. **Повторний вибір:** Якщо ви зміните частоту тут і натиснете кнопку знову, вона застосується до всіх запитів, навіть якщо ви їх змінювали вручну.
                """)
    ...
    for idx, k in enumerate(keywords, start=1):
        with st.container(border=True):
            ...
            with c4:
                ...
                # 🔥 FIX: Ініціалізуємо змінну перед використанням, щоб не було UnboundLocalError
                new_auto = is_auto 
