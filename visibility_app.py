...
def show_keywords_page():
    ...
    ВЕРСІЯ: FORCE STATE SYNC.
    1. Жорстка синхронізація стану toggle з базою даних.
    2. Виправлено візуальне "залипання" чекбоксів при масових діях.
    3. Логіка: Спочатку оновлюємо БД -> Чистимо кеш віджетів -> Перезавантажуємо -> Відображаємо актуальне.
    """
    import pandas as pd
    import streamlit as st
    from datetime import datetime
            ...
            if not allow_cron_global:
                ...
                with c_btn:
                    ...
                    if st.button("✅ Застосувати частоту та Увімкнути", type="primary", use_container_width=True):
                        try:
                            # 1. Масове оновлення в БД
                            supabase.table("keywords").update({
                                "is_auto_scan": True,
                                "frequency": selected_freq_db
                            }).eq("project_id", proj["id"]).execute()
                            
                            # 2. 🔥 ВАЖЛИВО: ОЧИЩЕННЯ СТАНУ ВІДЖЕТІВ
                            # Видаляємо всі ключі, пов'язані з перемикачами, щоб Streamlit їх перечитав з БД
                            for key in list(st.session_state.keys()):
                                if str(key).startswith("auto_") or str(key).startswith("freq_"):
                ...
                if st.button("⛔ Вимкнути автосканування для всіх", use_container_width=True):
                     try:
                        # 1. Масове вимкнення в БД
                        supabase.table("keywords").update({
                            "is_auto_scan": False
                        }).eq("project_id", proj["id"]).execute()

                        # 2. 🔥 ВАЖЛИВО: ОЧИЩЕННЯ СТАНУ ВІДЖЕТІВ
                        # Щоб перемикачі візуально стали False
                        for key in list(st.session_state.keys()):
                            if str(key).startswith("auto_") or str(key).startswith("freq_"):
                                del st.session_state[key]
                ...
                st.markdown("""
                **ℹ️ Як це працює:**
                1. **✅ Застосувати:** Активує автозапуск для всіх запитів і встановлює обрану частоту. Всі чекбокси стануть активними.
                2. **⛔ Вимкнути всі:** Вимикає автозапуск для всіх запитів. Всі чекбокси стануть неактивними.
                3. **Синхронізація:** Чекбокси в таблиці нижче завжди відображають актуальний стан з бази даних.
                """)
    ...
    for idx, k in enumerate(keywords, start=1):
        with st.container(border=True):
            ...
            with c4:
                ...
                # Читаємо актуальний стан з БД (він оновиться після rerun)
                ...
                with cron_c1:
                    if allow_cron_global:
                        ...
                        # Якщо перемикач змінювався користувачем вручну, це буде тут.
                        # Якщо ми натиснули Bulk Update, ключа в session_state не буде, і він візьме is_auto_db
                        ...
                        if new_auto != is_auto_db:
                            ...
                            # Тут можна не робити rerun, бо toggle вже перемкнувся візуально
                            # Але якщо хочемо, щоб з'явився селектбокс, треба rerun
                            time.sleep(0.1)
                            st.rerun()
                    else:
                ...
                with cron_c2:
                    # Показуємо частоту ТІЛЬКИ якщо увімкнено (з БД або щойно перемкнули)
                    if (is_auto_db or new_auto) and allow_cron_global:
