...
# 🔴 ПРОДАКШН N8N ВЕБХУКИ (можна також взяти з st.secrets)
...
try:
    # ✅ беремо значення без ["url"] / ["key"], бо це вже рядки з secrets
...
def n8n_generate_prompts(brand: str, domain: str):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            # n8n може повернути просто список або обʼєкт з ключем prompts
...
def load_user_project(user_id: str) -> bool:
    """Завантажує перший проект користувача, якщо є."""
    try:
...
def get_user_role_and_details(user_id: str):
    """Отримує роль і базові дані профілю."""
    try:
...
def check_session():
    """Відновлення сесії за токеном у cookie."""
...
def login_user(email: str, password: str):
    """Логін користувача по email + пароль."""
    try:
        ...
        st.error("Помилка входу: невірний логін, пароль або непідтверджений email.")
...
def show_dashboard():
    ...
    with c2:
        time_range = st.selectbox(
            "Період:", ["Останні 7 днів", "Останні 30 днів"], index=0
        )
    st.markdown("---")
    ...
    with k3:
        with st.container(border=True):
            ...
            # TODO: коли будуть реальні дані sentiment – підтягнути з БД
            fig = go.Figure(
                data=[
                    go.Pie(
                        labels=["Pos", "Neu", "Neg"],
...
def sidebar_menu():
    with st.sidebar:
        ...
        # GPT Visibility – завжди останній
...
def main():
    ...
    # 1. Немає користувача – показуємо логін/реєстрацію
    if not st.session_state["user"]:
        login_page()

    # 2. Є користувач, але немає проекту (і не admin) – онбординг
    elif (
    ...
    # 3. Основна зона (дашборд і т.д.)
    else:
        if st.session_state["role"] == "admin" and not st.session_state.get(
            "current_project"
        ):
            # Адмін без обраного проекту – чекаємо вибору в сайдбарі
            pass
