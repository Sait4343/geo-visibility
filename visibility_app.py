    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if isinstance(data, list): 
                return data  # Якщо n8n повернув чистий список
...
def register_user(email, password, first, last):
    try:
        ...
        if res.user:
            ...
            # Якщо email confirmation вимкнено і є session – логінимо одразу
            if res.session:
                ...
                st.success(
                    ...
                    "і потім увійдіть на вкладці «Вхід»."
                )
            return True
    except Exception as e:
...
def onboarding_wizard():
    ...
    with st.container(border=True):
        ...
        # STEP 2 – бренд та домен (ОНОВЛЕНО: 4 поля)
        if step == 2:
            ...
            products = st.text_area("Продукти / Послуги (через кому або список)")
            
            if st.button("Згенерувати запити"):
                if brand and domain and industry and products:
                    # Зберігаємо тимчасові дані про проєкт
...
def show_dashboard():
    ...
    with c2: 
        time_range = st.selectbox("Період:", ["Останні 7 днів", "Останні 30 днів"])
    st.markdown("---")
...
def main():
    ...
    if not st.session_state['user']:
        ...
        if page == "Дашборд": 
            ...
            try:
                st.dataframe(pd.DataFrame(supabase.table('projects').select("*").execute().data), use_container_width=True)
            except Exception: 
